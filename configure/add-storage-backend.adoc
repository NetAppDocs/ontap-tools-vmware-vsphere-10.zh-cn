...
NOTE: 从ONTAP工具管理器或ONTAP工具API添加存储后端时、存储后端为全局后端。从vCenter Server API添加存储后端为本地后端。
例如、在多租户设置中、您可以全局添加存储后端(集群)并在本地添加SVM、以使用SVM用户凭据。
...
NOTE: 支持IPv4和IPv6管理生命周期。此外、还支持基于SVM用户的凭据以及管理生命周期。
...
NOTE: 您可以添加基于集群的凭据以及IPv4和IPv6管理LIF、也可以通过SVM的管理LIF提供基于SVM的凭据、以便直接添加SVM用户。
