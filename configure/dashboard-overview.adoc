...
对于增强型链接模式 (ELM) 设置，将出现 vCenter Server 选择下拉菜单，您可以选择所需的 vCenter Server 来查看与其相关的数据。此下拉菜单适用于该插件的所有其他列表视图。在一个页面中所做的 vCenter Server 选择将保留在该插件的各个选项卡中。image:../media/remote-plugin-dashboard.png["信息板概述"]
