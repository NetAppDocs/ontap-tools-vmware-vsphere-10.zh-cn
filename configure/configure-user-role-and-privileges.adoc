...
summary: 使用来自 ONTAP tools 和 ONTAP System Manager 的 JSON 文件配置存储后端的用户角色和权限。 
---
...
[role="lead"]
使用 ONTAP tools for VMware vSphere 和 ONTAP System Manager 为存储后端配置 ONTAP 用户角色和 Privileges。您可以使用提供的 JSON 文件分配角色，手动创建用户和角色，或为非管理员帐户应用所需的最低 Privileges。

.开始之前
* 从 ONTAP tools for VMware vSphere 下载 ONTAP Privileges 文件，下载地址为 _\https://<ONTAPtoolsIP>:8443/virtualization/user-privileges/users_roles.zip_。该 ZIP 文件包含两个 JSON 文件。配置 ASA r2 系统时，请使用特定于 ASA r2 的 JSON 文件。
+
...
.. 从下拉列表中选择产品功能为 *VSC、VASA Provider 和 SRA*。
+
...
=== 非管理员、全局范围的集群用户所需的最低权限列表

此页面列出了没有 JSON 文件的非管理员全局范围集群用户所需的最低 Privileges。如果集群在本地范围内，请使用 JSON 文件创建用户，因为 ONTAP tools for VMware vSphere 在 ONTAP 上进行配置时不仅仅需要读取 Privileges。
