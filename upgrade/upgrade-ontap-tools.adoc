...
sidebar: sidebar 
keywords: upgrade 
summary: 从适用于VMware vSphere 10.0和10.1的ONTAP工具升级到10.2后、系统将创建一个恢复卷、并将所有必要的详细信息输入到恢复卷中。 
---
= 升级到最新版本的 ONTAP 工具
:allow-uri-read: 
:icons: font
...
[role="lead"]
从适用于VMware vSphere 10.0和10.1的ONTAP工具升级到10.2后、系统将创建一个恢复卷、并将所有必要的详细信息输入到恢复卷中。如果需要恢复配置、可以使用恢复卷恢复适用于VMware vSphere的ONTAP工具设置。如果您要对适用于VMware vSphere 10.2的ONTAP工具执行修补程序级别升级、则会使用同一恢复卷并刷新详细信息。HA和非HA部署均支持升级。
