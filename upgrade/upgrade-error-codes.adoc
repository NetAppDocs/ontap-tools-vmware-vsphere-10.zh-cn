...
| 084 | 将JobManager重新更改为生产失败 | 请执行以下步骤来恢复/完成升级。1.启用 Diagnostic Shell 2.运行命令：_sudo perl /home/maint/scripts/post-deploy-upgrade.pl --postupgrade_ 3.请查看 /var/log/post-deploy-upgrade.log 中的日志 
...
| 090 | 存储后端和 vCenter 的证书验证失败 | 检查升级日志和日志文件 /var/log/cert_validation_error.log 并重试升级。 
