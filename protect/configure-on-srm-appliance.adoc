...
sidebar: sidebar 
keywords:  
summary: '部署SRM设备后、您应在此SRM设备上配置SRA。成功配置 SRA 后， SRM 设备便可与 SRA 进行通信以进行灾难恢复管理。您应将适用于VMware vSphere凭据的ONTAP工具\(IP地址和管理员密码\)存储在SRM设备中、以实现SRM设备与SRA之间的通信。' 
---
= 在SRM设备上配置SRA
:allow-uri-read: 
:icons: font
...
[role="lead"]
部署SRM设备后、您应在此SRM设备上配置SRA。成功配置 SRA 后， SRM 设备便可与 SRA 进行通信以进行灾难恢复管理。您应将适用于VMware vSphere凭据(IP地址)的ONTAP工具存储在SRM设备中、以实现SRM设备与SRA之间的通信。
