...
NOTE: 存储复制适配器 (SRA) 支持具有自动故障转移双工类型的一个同步关系的扇出以及一致性组上的异步关系SnapMirror 。但是，不支持在一致性组上使用两个异步SnapMirror进行扇出或在卷上使用扇出 SnapMirror 进行扇出。这些扇出限制中不考虑 Vault 类型的SnapMirror关系。
