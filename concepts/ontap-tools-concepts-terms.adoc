...
一致性组是作为单个单元进行管理的卷的集合。一致性组跨存储单元和卷进行同步以确保数据一致性。在ONTAP中，它们为跨多个卷的应用程序工作负载提供了轻松的管理和保护保障。详细了解 https://docs.netapp.com/us-en/ontap/consistency-groups/index.html["一致性组"]。
...
vVols为虚拟机使用的存储提供了卷级抽象。它具有多种优点，并提供了使用传统 LUN 的替代方案。  vVol 数据存储通常与vVols容器的单个 LUN 相关联。
