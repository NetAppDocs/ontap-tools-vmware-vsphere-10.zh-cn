...
summary: 了解 ONTAP tools 如何创建和管理用于数据存储区访问的 igroup 和导出策略。 
---
= ONTAP tools 中的 Igroup 和导出策略工作流
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
使用此信息了解 ONTAP tools for VMware vSphere 如何创建和管理用于数据存储区访问的 igroup 和导出策略。

在适用于 VMware vSphere 9.x 的 ONTAP tools 中，igroup 是在平面结构中创建和管理的，其中 vCenter 中的每个数据存储区都与单个 igroup 相关联。此模型限制了跨多个数据存储区的 igroup 的灵活性和重用。ONTAP tools for VMware vSphere 10.4 引入了嵌套 igroup，其中 vCenter 中的每个数据存储区都与父 igroup 相关联，而每个主机都链接到该父 igroup 下的子 igroup。您可以使用用户定义的名称定义自定义父 igroup，以便跨数据存储区重用，从而使 igroup 管理更加简便。使用此信息了解 ONTAP tools for VMware vSphere 如何通过 igroup 工作流管理 LUN 和数据存储库。不同的工作流会生成不同的 igroup 配置，如以下示例所示：


NOTE: 所提及的名称仅用于说明目的，并不代表真实的 igroup 名称。 ONTAP工具管理的 igroup 使用前缀“otv_”。自定义 igroup 可以赋予任意名称。
...
ONTAP为 DS1 创建父 igroup DS1Igroup，并将子 igroup host1Igroup 映射到 lun1。系统始终将 LUN 映射到子 igroup。
...
ONTAP tools for VMware vSphere 创建子 igroup host2Igroup 并将其添加到现有父 igroup DS1Igroup。
...
ONTAP tools for VMware vSphere从层次结构中删除 host1Igroup。系统不会明确删除子 igroup。它会在以下两种情况下删除它们：
...
除非另一个数据存储重新使用父 igroup，否则父 igroup 和子 igroup 都会被删除。子 igroup 未被明确删除
...
ONTAP tools for VMware vSphere 9.x 版本不支持分层 igroup。在迁移到 10.3 或更高版本期间，必须将 igroup 规范化为层次结构。
...
从 ONTAP tools 10.5P2 开始，迁移的 igroup 在不再使用时将被删除。在早期版本中，迁移的 igroup 从未被删除。请注意以下几点：

* 已迁移的 igroup 无法跨数据存储区重复使用。
* 自定义 igroups（用户定义）永远不会被删除，并且仍然可以重复使用。
...
导出策略控制ONTAP tools for VMware vSphere中的 NFS 数据存储访问和客户端权限。导出策略在ONTAP系统中创建和管理，并可与 NFS 数据存储库一起使用来强制执行访问控制。每个导出策略均由指定允许访问的客户端（IP 地址或子网）和授予的权限（只读或读写）的规则组成。
...
当您从 ESXi 主机删除或卸载 NFS 数据存储区时，ONTAP tools for VMware vSphere 会从导出策略中删除主机的 IP 地址。如果没有其他主机正在使用该导出策略，它将被删除。当您删除 NFS 数据存储区时，如果该导出策略未被任何其他数据存储区重用，ONTAP tools for VMware vSphere 将删除与该数据存储区关联的导出策略。如果导出策略被重用，则保留主机 IP 地址，不做任何更改。当您删除数据存储区时，ONTAP tools 将从导出策略中删除主机 IP 地址，并在 ONTAP 需要访问时分配默认导出策略。
