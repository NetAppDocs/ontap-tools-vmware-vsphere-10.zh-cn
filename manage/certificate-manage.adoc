...
summary: 管理 ONTAP tools 和 VASA Provider 证书、续订默认证书或替换为自定义 CA 证书。 
---
= 管理 ONTAP tools 证书
:allow-uri-read: 
:icons: font
...
[role="lead"]
默认情况下，ONTAP tools 和 VASA Provider 使用自签名证书。在 ONTAP tools Manager 中，您可以续订该证书或将其替换为自定义 CA 证书。多 vCenter 部署需要自定义 CA 证书。


WARNING: 如果证书过期而未及时续订，ONTAP tools 将停止运行。在到期前续订证书，以保持连续运行并防止服务中断。
...
NOTE: ONTAP tools IP 地址应映射到完全限定域名 (FQDN)。证书应在使用者替代名称 (SAN) 字段中包含相同的 FQDN。
...
.升级ONTAP工具证书
--
ONTAP tools 选项卡显示证书类型（自签名或 CA 签名）和域名等详细信息。在部署期间，ONTAP tools 默认会生成自签名证书。您可以续订证书或将其升级为 CA 签名的证书。
...
NOTE: 在证书续订完成之前，系统处于脱机状态，并且 ONTAP tools Manager 会将您注销。
...
.升级VASA Provider证书
--
ONTAP tools for VMware vSphere 默认对 VASA Provider 使用自签名证书。使用该证书，您只能管理一个用于 vVols 数据存储的 vCenter Server 实例。当您管理多个 vCenter Server 实例并希望在这些实例上启用 vVols 功能时，需要将自签名证书更改为自定义 CA 证书。
...
WARNING: 此操作会重新启动所有服务，并可能导致停机或中断。若要继续，请键入 `y`以确认警告，然后刷新过程将开始。
