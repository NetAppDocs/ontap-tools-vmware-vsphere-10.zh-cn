...
[role="lead"]
如果将数据存储区从非 ONTAP tools 管理的环境移动到由 ONTAP tools 管理的环境，请了解 igroups 的行为方式。ONTAP tools for VMware vSphere 使用嵌套的 igroup 模型，这有助于解释早期版本和当前实现之间的行为差异。

从适用于 VMware vSphere 10.4 的 ONTAP tools 开始，ONTAP tools 会自动创建和维护 ONTAP 和 vCenter 对象，从而简化 VMware 数据中心环境中的数据存储管理。从适用于 VMware vSphere 10.5 P2 的 ONTAP tools 开始，ONTAP tools 还会在迁移的 igroup 不再使用时自动将其删除。
...
例如，如果创建 datastore1 并将其挂载到主机 1、2 和 3 上，然后创建 datastore2 并将其挂载到主机 3、4 和 5 上，则 ONTAP tools 会重用主机级 igroup 以实现高效管理。
...
如果您直接在 ONTAP 系统和 VMware 环境中创建 igroup 和数据存储区，ONTAP tools 最初不会管理这些对象，从而导致 igroup 结构扁平化。
