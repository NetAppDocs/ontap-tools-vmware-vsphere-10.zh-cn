...
[role="lead"]
如果您同时管理 ONTAP 工具虚拟机和 ONTAP 存储系统，则需要了解 igroups 的行为方式，特别是在将数据存储从非由 ONTAP 工具管理的环境移动到由 ONTAP 工具管理的环境时。ONTAP tools for VMware vSphere 使用嵌套的 igroup 模型来表示和管理 igroup，了解此模型有助于解释早期版本与当前实现之间的行为差异。
