...
支持没有内存的虚拟机快照。如果虚拟机具有带内存的快照，则无法保护该虚拟机。

无法保护包含内存快照的未受保护虚拟机。在对虚拟机启用保护之前，请删除内存快照。

对于具有 ASA r2 存储类型的 Windows VM，虚拟机快照为只读。当您打开虚拟机时，VASA Provider 将从只读快照创建 LUN 并启用 IOPS。当您关闭虚拟机时，VASA Provider 会删除 LUN 并禁用 IOPS。
