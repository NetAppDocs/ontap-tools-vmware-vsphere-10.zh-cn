...
. 从URL中选择OVA文件或浏览到其位置，然后选择*Next*。
. 从内容库中部署适用于VMware vSphere的ONTAP工具：
+
.. 转到内容库、然后选择要部署的库项目。
.. 选择*Actions*>*New VM from this temple*


. 在*选择名称和文件夹*字段中，输入虚拟机名称并选择其位置。如果选中标记为*自定义此虚拟机的硬件*的复选框，则在进入*Ready to Complete*(准备完成)窗口之前，还会启用一个名为*自定义硬件*的步骤。
