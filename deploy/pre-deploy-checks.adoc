...
| ONTAP 工具应用程序的 IP 地址。这是访问 ONTAP 工具管理器 Web 界面（负载平衡器）的 IP 地址，位于 `\https://<ip>:8443/virtualization/ui/` |  


| ONTAP tools 用于内部通信的虚拟 IP 地址。此 IP 地址用于在具有多个 ONTAP tools 实例的设置中进行内部通信。此 IP 地址不应与 ONTAP tools 应用程序的 IP 地址相同。（Kubernetes Control Plane） |  
...
| ONTAP 工具管理节点的 IPv4 地址。它是管理网络上节点管理接口的唯一 IPv4 地址。这用于连接到 ONTAP 工具设备，以便通过 SSH 进行远程诊断访问。 |  
