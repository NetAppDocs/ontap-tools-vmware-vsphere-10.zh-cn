...
. 仅当触发数据存储发现作业后，从ONTAP tools for VMware vSphere迁移的 NFS 和 VMFS 数据存储才会在ONTAP tools for VMware vSphere中可见，这可能需要长达 30 分钟才能完成。验证数据存储库是否在 VMware vSphere 插件用户界面页面的ONTAP工具概览页面上可见。
