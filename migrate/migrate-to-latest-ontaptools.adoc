...
NOTE: 建议仅在适用于VMware vSphere 9.xx的ONTAP工具设置仅提供VASA Provider功能时才迁移此设置。


NOTE: 从适用于VMware vSpher 9.x的ONTAP工具迁移到10.3之后、采用NVMe/FC协议的Vvol数据存储库将无法工作、因为ONTAP工具10.3仅支持对VMFS数据存储库使用NVMe-oF。

* 关于此任务 *

支持从适用于VMware vSphere 9.12D1和9.13D2的ONTAP工具迁移到10.3版。
...
. 只有在触发数据存储库发现作业后、从适用于VMware vSphere 9.xx的ONTAP工具迁移的NFS和VMFS数据存储库才会显示在适用于VMware vSphere 10.3的ONTAP工具中、此作业可能需要长达30分钟才能触发。验证这些数据存储库是否显示在适用于VMware vSphere的ONTAP工具插件用户界面页面的概述页面中。
...
.. 单击省略号菜单，然后单击*Reset configuration*
.. 单击省略号菜单并选择*Delete*
+
在保护站点和恢复站点上执行这些步骤。
...
从适用于VMware vSphere的旧版ONTAP工具迁移到10.3版后、请重新扫描SRA适配器、以验证是否已在VMware Live Site Recovery存储复制适配器页面上更新详细信息。
